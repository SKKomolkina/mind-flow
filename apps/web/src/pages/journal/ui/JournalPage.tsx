...
export const JournalPage = () => {
  ...
  if (loading) return <div
    className="p-20 text-center font-black text-slate-300 uppercase tracking-widest">Загрузка...</div>;
          ...
          <div className="space-y-10 mt-16">
            {entries.map((entry: any) => (
              <div key={entry.id}
                ...
                <div className="mb-6">
                  <span
                    className="text-[10px] font-black text-blue-600 uppercase tracking-tighter mb-1 block">Ситуация</span>
                      ...
                      return (
                        <div key={emo.id}>
                          ...
                          <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden relative">
                            ...
                            {/* Яркая полоска (СТАЛО) */}
                            <div className="absolute h-full bg-blue-600 shadow-[0_0_8px_rgba(37,99,235,0.4)]"
                                 style={{ width: `${after}%` }} />
                          </div>
                        </div>
                      );
                ...
                <div className="space-y-4 border-t border-slate-50 pt-6">
                  ...
                  {entry.rationalResponse && (
                    <div
                      className="p-5 bg-blue-600 rounded-2xl text-white text-sm font-bold shadow-lg shadow-blue-500/20">
                      {entry.rationalResponse}
                    </div>
                  )}
                  <div className="flex justify-between text-[10px] font-black uppercase text-slate-400">
                    <span>Вера: {entry.beliefBefore}% → <span
                      className="text-blue-600">{entry.beliefAfter}%</span></span>
                  </div>
                </div>
              </div>
            ))}
